...
export default function Header({path} : {path: string[]}){
    ...
    return (
        ...
        <header className="main-header">
          <button className="btn btn-ghost" style={{ display: "none" }} id="mobile-menu-btn">
            <Menu style={{ height: "20px", width: "20px" }} />
          </button>
          <div style={{ height: "1px", backgroundColor: "#e5e7eb", margin: "0 0.5rem" }}></div>
                            ...
                            path.map((newVal,newInd)  => (
                              <>
                                <div style={{display:"none"}}>
                                  {newVal}
                                  {newInd <= ind ? absolutePath += path[newInd] +"/":""}
                                </div>
                              </>
                            ))
                          }
